...
"""Shared deterministic feature extraction (training + runtime portable).

Design constraints:
- deterministic across platforms/runs (no PYTHONHASHSEED dependence)
- runtime needs only stdlib + numpy
- word n-grams hashed with zlib.crc32; char n-grams with a vectorized
  numpy polynomial rolling hash
"""
...
CHAR_BINS = 1 << 16
CHAR_NS = (3, 4)
CHAR_CAP = 4096  # hash char-grams over the first 4096 chars only
...
def dense_features(text: str, message_count: int = 1) -> List[float]:
    ...
    # pad the "hand" block to 46 dims for future additions
...
def featurize_batch(texts: Sequence[str], message_counts: Sequence[int] | None = None):
    """Return dense (N,D) float64 + word/char scipy CSR (L2-normalized rows)."""

    from scipy import sparse

    if message_counts is None:
        message_counts = [1] * len(texts)
    dense = np.asarray(
        [dense_features(t, m) for t, m in zip(texts, message_counts)],
        dtype=np.float64,
    )
    ...
    word = to_csr([word_gram_hash(t) for t in texts], WORD_BINS)
    char = to_csr([char_gram_hash(t) for t in texts], CHAR_BINS)
